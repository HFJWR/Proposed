...
def  detect_community(retweets, rnd_state):
    ...
    mention_id = list(retweets['ref_author'])
    node_list = list(set(author_id+mention_id))

    # Edge list
    edge_list = list(zip(retweets["author_id"], retweets["ref_author"]))
    ...
    print(f"LCC interaction: ",len(edges))
    ...
    # LCC
    retweets_LCC = pd.DataFrame(edges, columns=['author_id', 'ref_author'])
